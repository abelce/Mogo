 ...
 * File: render.go
 * Project: Mogo
 * File Created: Thursday, 16th May 2019 2:40:42 pm
 * Author: zxtang (1061225829@qq.com)
 * -----
 * Last Modified: Thursday, 16th May 2019 2:40:42 pm
...
func parseTemplate(root, tpl string, cfg *yaml.File) *template.Template {
	themeFold, err := cfg.Get("theme")
	if err != nil {
		log.Println("get theme error!check gloabl.yml")
		os.Exit(1)
	}
...
func processArticleFile(filePath, fileName string) (string, ArticleConfig, error) {
	...
	var ct int = 0
	var markdownStr, yamlStr string
	for {
